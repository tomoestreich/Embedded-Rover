...
class ConnHandler:
	...
	def createSocket(self):
		...
		try:
			...
			print boards[str(self.server[1])], 'create socket:', str(err)
			return False
		return True
	...
	def close(self):
		...
		try:
			...
			print boards[str(self.server[1])], 'close:', str(err)
		self.connected = False
		...
		while True:
			...
			if len(txQueues[str(self.server[1])]):
				try:
					...
					#print boards[str(self.server[1])], 'TX:', str(err)
					continue
	...
	def receiveHandler(self):
		...
		print "[%s] Connection found: %s:%s" \
				%(boards[str(self.server[1])], self.addr[0], self.addr[1])			
		
		# Run "forever"
		while True:
			# Check pings for maintaining connectivity
			if self.connected and (time.time()-self.lastPing > self.timeout):
				print "[%s] Disconnected from client." %(\
						boards[str(self.server[1])])
			...
			if (msg == '*HEL') or (msg == 'LO*') or not len(msg):
				...
				print '[%s] Start signal received.' %boards[str(self.server[1])]
	...
	def routeMessage(self, msg):
		...
		# Route Decisions to Pi Control thread
		if (src == THESEUS_CONTROL and dst == THESEUS_LINE) or \
			(src == MINOTAUR_CONTROL and dst == MINOTAUR_LINE):
			txQueues["2005"].append(msg)
			txQueues["2005"].append(msg)
	...
	def logMessage(self, msg):
		...
		print "[%s] %s" %(boards[str(self.server[1])], mStr)
		self.log.write("[%s] %s\n" % (timeStr, mStr))
	...
	def invalidMsg(self, message):
		...
		try:
			...
			print boards[str(self.server[1])], 'validity:', str(err)
			return 1
		return 0
